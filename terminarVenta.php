...
$sentenciastock = $base_de_datos->prepare("UPDATE productos SET cantidadd = cantidadd - ? WHERE id_producto = ?;");
foreach ($_SESSION["carrito"] as $producto) {
	$total += $producto->total;
	$sentencia->execute([$producto->id_producto, $idVenta, $producto->cantidad]);
	$sentenciastock->execute([$producto->cantidad, $producto->id_producto]);
}
